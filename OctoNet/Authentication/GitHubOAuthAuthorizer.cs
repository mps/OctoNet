...
namespace OctoNet.Authentication
{
    public class GitHubOAuthAuthorizer : IGitHubOAuthAuthorizer
    {
        ...
        #region IGitHubOAuthAuthorizer Members
                                             ...
                                             params Scope[] scopes)
        {
            ...
            string url = string.Format("{0}?client_id={1}&redirect_uri={2}{3}",
                                        ...
                                        Action<GitHubException> onError)
        {
            ...
            IRestClient client = _factory.CreateRestClient(Constants.AuthorizeUrl);
                ...
                (r, h) =>
                    {
                        ...
                        string[] parameters = response.Content.Split('&');
                        string accessToken = parameters.Where(p => p.StartsWith("access_token="))
        ...
        private string BuildScopeText(Scope[] scopes)
        {
            ...
            foreach (Scope scope in scopes.Distinct())
            {
                if (isFirstScope)
                {
                    isFirstScope = false;
